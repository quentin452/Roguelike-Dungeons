...
public class CommandSpawnDungeon extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args) {
        ...
        if (!ap.hasEntry(0)) {
            sender.addChatMessage(
                    new ChatComponentText(
                            TextFormat.apply("Usage: roguelike [dungeon | give | config]", TextFormat.GRAY)));
            return;
        }
        ...
        if (ap.match(0, "config")) {
            if (!ap.hasEntry(1)) {
                sender.addChatMessage(
                        new ChatComponentText(TextFormat.apply("Usage: roguelike config reload", TextFormat.GRAY)));
                return;
            }
            if (ap.match(1, "reload")) {
                RogueConfig.reload(true);
                sender.addChatMessage(
                        new ChatComponentText(TextFormat.apply("Success: Configurations Reloaded", TextFormat.GREEN)));
                return;
            }
            return;
        }
        ...
        if (ap.match(0, "settings")) {
            if (!ap.hasEntry(1)) {
                sender.addChatMessage(
                        new ChatComponentText(
                                TextFormat.apply("Usage: roguelike settings [reload | list]", TextFormat.GRAY)));
                return;
            }
            if (ap.match(1, "reload")) {
                Dungeon.initResolver();
                sender.addChatMessage(
                        new ChatComponentText(TextFormat.apply("Success: Settings Reloaded", TextFormat.GREEN)));
                return;
            }
            if (ap.match(1, "list")) {
                sender.addChatMessage(
                        new ChatComponentText(TextFormat.apply(Dungeon.settingsResolver.toString(), TextFormat.GREEN)));
                return;
            }
        ...
        if (ap.match(0, "give")) {
            if (!ap.hasEntry(1)) {
                sender.addChatMessage(
                        new ChatComponentText(TextFormat.apply("Usage: roguelike give novelty_name", TextFormat.GRAY)));
                return;
            }
            ...
            sender.addChatMessage(
                    new ChatComponentText(
                            TextFormat.apply("Success: Given " + item.getDisplayName(), TextFormat.GREEN)));
            return;
        }
        ...
        if (ap.match(0, "dungeon")) {
            ...
            if (!ap.hasEntry(1)) {
                sender.addChatMessage(
                        new ChatComponentText(
                                TextFormat.apply("Usage: roguelike dungeon {X Z | here} [setting]", TextFormat.GRAY)));
                return;
            }
            ...
            if (ap.match(1, "here")) {
                ...
                try {
                    ...
                    sender.addChatMessage(
                            new ChatComponentText(TextFormat.apply("Failure: Cannot find player", TextFormat.RED)));
                    return;
                }
                x = (int) player.posX;
                z = (int) player.posZ;
                ...
                if (ap.hasEntry(2)) {
                    ...
                    try {
                        ...
                        sender.addChatMessage(
                                new ChatComponentText(
                                        TextFormat.apply(
                                                "Failure: Third argument must be a whole number",
                                                TextFormat.RED)));
                        return;
                    }
                    ...
                    if (num <= 0) {
                        sender.addChatMessage(
                                new ChatComponentText(
                                        TextFormat.apply(
                                                "Failure: Third argument must be greater than zero.",
                                                TextFormat.RED)));
                        return;
                    }
                    ...
                    sender.addChatMessage(
                            new ChatComponentText(
                                    TextFormat.apply(
                                            "Success: Dungeons generated all over the place!",
                                            TextFormat.GREEN)));
                    return;
                }
                ...
                try {
                    sender.addChatMessage(
                            new ChatComponentText(
                                    TextFormat.apply(
                                            "Success: Dungeon generated at " + toGenerate.getPosition().toString(),
                                            TextFormat.GREEN)));
                } catch (Exception e) {
                    sender.addChatMessage(
                            new ChatComponentText(
                                    TextFormat.apply("Failure: Unable to generate dungeon", TextFormat.RED)));
                }
                return;
                ...
                try {
                    ...
                    sender.addChatMessage(
                            new ChatComponentText(TextFormat.apply("Failure: Invalid Coords: X Z", TextFormat.RED)));
                    return;
                }
            ...
            if (settingName != null) {
                ...
                if (settings == null) {
                    sender.addChatMessage(
                            new ChatComponentText(
                                    TextFormat.apply("Failed: " + settingName + " not found.", TextFormat.RED)));
                    return;
                }
                ...
                try {
                    sender.addChatMessage(
                            new ChatComponentText(
                                    TextFormat.apply(
                                            "Success: \"" + settingName
                                                    + "\" Dungeon generated at "
                                                    + dungeon.getPosition().toString(),
                                            TextFormat.GREEN)));
                } catch (Exception e) {
                    sender.addChatMessage(
                            new ChatComponentText(
        ...
        sender.addChatMessage(
                new ChatComponentText(TextFormat.apply("Usage: roguelike [dungeon | give | config]", TextFormat.GRAY)));
